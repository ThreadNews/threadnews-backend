...
def like_article(articleId):
    ...
    if request.method == "POST":
        ...
        current_user = get_jwt_identity()
        if data["action"] == "add":
            database_client.push_new_like(
                current_user["user_id"], articleId, articleId
            )  # need to verify that this is being used properly
        if data["action"] == "delete":
            database_client.delete_like(
                current_user["user_id"], data["article_id"]
            )  # need to verify that this is being used properly
        return 200
